...
import { Calendar, MapPin, Users, Star, ChevronRight, Shield, Award, Compass, Play, ArrowRight, Mountain, Globe, Trophy, Clock, Edit, Settings, ChevronDown } from 'lucide-react';
...
function FAQSection() {
  ...
  const faqs = [
    { q: "What experience level do I need?", a: "Our tours cater to various skill levels. Most require basic riding experience with a valid motorcycle license. We'll help match you with the right tour based on your experience." },
    { q: "What's included in the price?", a: "Motorcycle rental, accommodation, meals (as specified), guide, support vehicle, fuel, and all necessary permits. International flights and personal expenses are not included." },
    ...
    { q: "What should I bring?", a: "We provide riding gear including helmet, jacket, and gloves. You should bring personal clothing for varying weather, any medications, camera, and a sense of adventure! We'll send a detailed packing list." },
    { q: "Can I customize my trip?", a: "Absolutely! We specialize in custom itineraries. We can arrange additional days, alternative routes, or even multi-country adventures. Contact us to discuss your dream trip." }
  ];
    ...
    <section className="py-20 bg-gray-50">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="space-y-4">
          {faqs.map((faq, i) => (
            <div key={i} className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 transition hover:shadow-md">
              <button
              ...
              {openIndex === i && (
                <div className="px-6 pb-5 text-gray-600 border-t border-gray-100 pt-4 bg-gray-50/50">{faq.a}</div>
              )}
            </div>
          ))}
        </div>
...
function EditButton({ section, label }: { section: string; label?: string }) {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-1 bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-blue-700 transition shadow-lg z-50 pointer-events-auto"
...
function HomepageEditPanel() {
  ...
  return (
    <div className="fixed top-24 right-4 z-50 bg-white rounded-xl shadow-2xl border border-gray-200 p-4 w-64 pointer-events-auto">
      <div className="flex items-center gap-2 mb-4 pb-3 border-b">
        <Settings className="text-blue-600" size={18} />
        <h3 className="font-semibold text-gray-900">Edit Homepage</h3>
      </div>
      <div className="space-y-2">
        <button onClick={() => navigate('/admin?tab=homepage&section=hero')} className="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 text-sm text-gray-700 flex items-center justify-between group">
          <span>Hero Section</span><Edit size={14} className="text-gray-400 group-hover:text-blue-600" />
        </button>
        <button onClick={() => navigate('/admin?tab=homepage&section=stats')} className="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 text-sm text-gray-700 flex items-center justify-between group">
          <span>Statistics</span><Edit size={14} className="text-gray-400 group-hover:text-blue-600" />
        </button>
        <button onClick={() => navigate('/admin?tab=homepage&section=featured')} className="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 text-sm text-gray-700 flex items-center justify-between group">
          <span>Featured Tours</span><Edit size={14} className="text-gray-400 group-hover:text-blue-600" />
        </button>
        <button onClick={() => navigate('/admin?tab=homepage&section=whyChooseUs')} className="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 text-sm text-gray-700 flex items-center justify-between group">
          <span>Why Choose Us</span><Edit size={14} className="text-gray-400 group-hover:text-blue-600" />
        </button>
        <div className="pt-2 border-t mt-2">
          <button onClick={() => navigate('/admin?tab=theme')} className="w-full text-left px-3 py-2 rounded-lg hover:bg-blue-50 text-sm text-blue-600 font-medium flex items-center justify-between">
            <span>🎨 Colors & Fonts</span><ChevronRight size={14} />
          </button>
        </div>
      </div>
    </div>
    ...
    <Link to={`/tours/${tour?.slug || ''}`} className="group bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${tour?.difficulty === 'Expert' ? 'bg-red-500' : tour?.difficulty === 'Challenging' ? 'bg-orange-500' : tour?.difficulty === 'Moderate' ? 'bg-yellow-500' : 'bg-green-500'} text-white`}>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="relative">
            ...
            <div className="rounded-2xl overflow-hidden shadow-2xl">
              <img src={tour?.heroImage || 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800'} alt={tour?.title || 'Tour'} className="w-full h-96 object-cover" />
            </div>
            <div className="absolute -bottom-6 -right-6 bg-white rounded-xl shadow-xl p-4">
              <div className="flex items-center gap-3">
                <div className="text-amber-500">{[...Array(5)].map((_, i) => <Star key={i} size={16} className="inline fill-amber-500" />)}</div>
                <span className="text-gray-600 text-sm">4.9/5 Rating</span>
              </div>
            </div>
          </div>
          <div className="text-white">
            <span className="inline-block bg-amber-500/20 text-amber-400 px-4 py-1 rounded-full text-sm font-semibold mb-4">⭐ Highly Recommended</span>
            ...
            <div className="flex items-center gap-6">
              <div>
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  {tour?.originalPrice && <span className="text-gray-500 line-through">${tour.originalPrice.toLocaleString()}</span>}
                </div>
              </div>
              <Link to={`/tours/${tour?.slug || ''}`} className="bg-amber-500 text-white px-8 py-4 rounded-xl font-semibold hover:bg-amber-600 transition flex items-center gap-2">
    ...
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">Join our upcoming expeditions and ride with fellow adventurers from around the world</p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {toursWithDates.map((tour, i) => {
            ...
            const countries = tour?.countries || [];
            return (
              <Link key={`${tour?.id || i}-${i}`} to={`/tours/${tour?.slug || ''}`} className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl hover:bg-amber-50 hover:shadow-lg transition group">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-semibold text-gray-900 truncate group-hover:text-amber-600 transition">{tour?.title || 'Tour'}</h4>
        ...
        <div className="text-center mt-8">
          <Link to="/tours" className="inline-flex items-center gap-2 text-amber-600 font-semibold hover:text-amber-700">View All Tours <ArrowRight size={18} /></Link>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="py-20 bg-gray-900 relative overflow-hidden">
      <div className="absolute inset-0 opacity-30">
        <img src="https://images.unsplash.com/photo-1568772585407-9361bd37ec91?w=1600&h=900&fit=crop" alt="Motorcycle adventure" className="w-full h-full object-cover" />
      </div>
      <div className="relative max-w-4xl mx-auto px-4 text-center">
        <span className="inline-block bg-amber-500/20 text-amber-400 px-4 py-1 rounded-full text-sm font-semibold mb-6">Watch Our Story</span>
        <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">Experience the Thrill of<br/><span className="text-amber-500">Himalayan Riding</span></h2>
        <p className="text-gray-300 text-lg mb-8 max-w-2xl mx-auto">Join us on an unforgettable journey through the world's highest motorable roads, ancient monasteries, and breathtaking landscapes.</p>
        <button className="inline-flex items-center gap-3 bg-white text-gray-900 px-8 py-4 rounded-full font-semibold hover:bg-amber-500 hover:text-white transition group">
          <div className="w-12 h-12 bg-amber-500 group-hover:bg-white rounded-full flex items-center justify-center transition"><Play className="text-white group-hover:text-amber-500 fill-current" size={20} /></div>
          Watch Video
        </button>
      </div>
    </section>
    ...
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">Ride the best adventure motorcycles in the world, meticulously maintained for the toughest terrains</p>
        </div>
            ...
            <div key={bike.id} className="group bg-gray-50 rounded-2xl overflow-hidden hover:shadow-xl transition-all">
              ...
              <div className="p-5">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 mb-4">
                  <div><span className="text-gray-400">Power:</span> {bike.power}</div>
                  <div><span className="text-gray-400">Weight:</span> {bike.weight}</div>
                </div>
                <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                  <div><span className="text-xs text-gray-400">From</span><p className="text-amber-600 font-bold">${bike.rentalPrice}/day</p></div>
                  <span className="text-green-600 text-sm font-semibold flex items-center gap-1"><span className="w-2 h-2 bg-green-500 rounded-full"></span> Available</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="text-center mt-12">
          <p className="text-gray-600 mb-4">All bikes come with full insurance, riding gear, and roadside assistance</p>
          <Link to="/tours" className="inline-flex items-center gap-2 bg-gray-900 text-white px-8 py-4 rounded-full font-semibold hover:bg-gray-800 transition">View Tours with Available Bikes <ChevronRight /></Link>
        </div>
      </div>
    </section>
  );
...
export function Home() {
  ...
  const displayTours = showFeaturedOnly ? featuredTours.slice(0, maxItems) : allPublishedTours.slice(0, maxItems);
      ...
      <section className="relative h-[90vh] -mt-28 flex items-center group/hero">
        ...
        <div className="relative max-w-7xl mx-auto px-4 pt-28">
          <div className="max-w-2xl">
            ...
            <h1 className="text-5xl md:text-7xl font-bold text-white mb-6 leading-tight">{homepage.hero.title}</h1>
            <p className="text-xl text-gray-300 mb-8">{homepage.hero.subtitle}</p>
            <div className="flex flex-wrap gap-4">
              <Link to={homepage.hero.ctaLink} className="bg-amber-600 text-white px-8 py-4 rounded-full font-semibold hover:bg-amber-700 transition text-lg flex items-center gap-2">{homepage.hero.ctaText} <ChevronRight /></Link>
              <a href={homepage.hero.secondaryCtaLink} className="border-2 border-white text-white px-8 py-4 rounded-full font-semibold hover:bg-white hover:text-gray-900 transition text-lg">{homepage.hero.secondaryCtaText}</a>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
              <div>
                ...
                <p className="text-gray-400 max-w-xl">Discover the world's most spectacular motorcycle routes through breathtaking landscapes</p>
              </div>
              <Link to="/destinations" className="mt-4 md:mt-0 inline-flex items-center gap-2 text-amber-500 font-semibold hover:text-amber-400 transition">View All Destinations <ArrowRight size={18} /></Link>
            </div>
        ...
        <section id="featured" className="py-20 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4">
            <div className="text-center mb-12">
              ...
              <p className="text-gray-600 max-w-2xl mx-auto">{homepage.featuredSection.subtitle}</p>
            </div>
      ...
      {homepage.ctaSection.enabled && (
        <section className="py-20 bg-gray-900 relative overflow-hidden">
          <div className="absolute inset-0 opacity-20">
            <img src={homepage.ctaSection.backgroundImage} alt="Background" className="w-full h-full object-cover" />
          </div>
          <div className="relative max-w-4xl mx-auto px-4 text-center">
            <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">{homepage.ctaSection.title}</h2>
            <p className="text-xl text-gray-300 mb-8">{homepage.ctaSection.subtitle}</p>
            <div className="flex flex-wrap justify-center gap-4">
              <Link to={homepage.ctaSection.ctaLink} className="bg-amber-600 text-white px-8 py-4 rounded-full font-semibold hover:bg-amber-700 transition text-lg">{homepage.ctaSection.ctaText}</Link>
              ...
              {homepage.testimonials.items.map((review, i) => (
                <div key={i} className="bg-white p-8 rounded-2xl shadow-lg">
                  <div className="flex gap-1 mb-4">
                    {[...Array(review.rating)].map((_, j) => <Star key={j} size={20} className="fill-amber-500 text-amber-500" />)}
                  </div>
